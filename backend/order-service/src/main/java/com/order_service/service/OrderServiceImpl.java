...
import com.order_service.client.ShopServiceClient;
...
public class OrderServiceImpl implements OrderService {
    ...
    /**
     * Retrieves an order by its unique identifier (read-only, no status change).
     * 
    ...
    public Order getOrderById(Long orderId) {
        log.debug("Get order by ID request received for order ID: {}", orderId);
        ...
        // Retrieve order from database (read-only, no status change)
        Order order = findOrderById(orderId);

        log.debug("Order found with status: {}", order.getOrderStatus());
